...
dag = DAG('dbm_daily_dag',
          default_args=ARGUMENTS.DEFAULT_ARGUMENTS,
          schedule_interval='30 14 * * 1-5', concurrency=2, catchup=False)

# start and end dummy operator task to indicate start and end process.
dummy_task_start = create_dummy_operator(dag, 'start_dbm_daily_task')
dummy_task_end = create_dummy_operator(dag, 'end_dbm_daily_task')
...
# DBM class intializtion
dbm_report_object = DbmReport()
...
create_directory_over_ftp_res = create_python_operator(
    dag,
    'create_dbm_directory_on_ftp',
    dbm_report_object.create_directory_on_ftp,
...
try:
    for each in input_config_list:
        ...
        download_dbm_report_task = create_python_operator(
            dag,
            f"download_dbm_report_task_{each['advertiser']}",
            dbm_report_object.download_segregate_and_upload_reports,
            op_kwargs={'report_config': each, 'platform': PLATFORM.DBM},
            provide_context=True
        )

        task_list.append(download_dbm_report_task)
