...
dag = DAG('ttd_daily_dag',
          default_args=ARGUMENTS.DEFAULT_ARGUMENTS,
          schedule_interval='00 15 * * *', concurrency=2, catchup=False)

# start and end dummy operator task to indicate start and end process.
dummy_task_start = create_dummy_operator(dag, 'start_ttd_daily_task')
dummy_task_end = create_dummy_operator(dag, 'end_ttd_daily_task')
...
# TTD class intializtion
ttd_report_object = TtdReport()
...
create_directory_over_ftp_res = create_python_operator(
    dag,
    'create_ttd_directory_on_ftp',
    ttd_report_object.create_directory_on_ftp,
...
try:
    for each in input_config_list:
        ...
        download_ttd_report_task = create_python_operator(
            dag,
            f"download_ttd_report_task_{each['advertiser']}",
            ttd_report_object.download_segregate_and_upload_reports,
            op_kwargs={'report_config': each, 'platform': PLATFORM.TTD},
            provide_context=True
        )

        task_list.append(download_ttd_report_task)
