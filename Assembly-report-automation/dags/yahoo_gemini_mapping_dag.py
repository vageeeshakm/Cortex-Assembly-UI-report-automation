...
# JOB WILL BE SCHEDULED ON EVERY MONDAY at 1PM IST
dag = DAG('yahoo_gemini_mapping_dag', default_args=ARGUMENTS.DEFAULT_ARGUMENTS, schedule_interval='00 13 * * 1', concurrency=2)
...
create_directory_over_ftp_res = create_python_operator(
    ...
    op_kwargs={'platform_type': PLATFORMTYPE.YAHOO_GEMINI_MAPPING})
...
create_consolidated_report = create_python_operator(
    dag,
    'create_yahoo_mapping_weekly_consolidated_report',
    consolidate_and_validate_object.download_individual_report_and_consolidate_for_platform,
    op_kwargs={'platform': PLATFORMTYPE.YAHOO_GEMINI_MAPPING},
...
email_consolidated_report = create_python_operator(
    dag,
    'email_yahoo_mapping_weekly_consolidated_report',
    consolidate_and_validate_object.download_consolidated_report_and_send_email,
    op_kwargs={'platform': PLATFORMTYPE.YAHOO_GEMINI_MAPPING})

# Read input from config file
input_config_list = read_json_file(file_path=PATH.YAHOO_MAPPING_JSON_FILE_PATH)
...
try:
    for each in input_config_list:
        download_pinterest_report_task = create_python_operator(
            dag,
            f"download_yahoo_gemini_mapping_report_task_{each['report_name']}",
            yahoo_gemini_object.download_combine_and_upload_reports,
            op_kwargs={'report_config': each, 'platform_type': PLATFORMTYPE.YAHOO_GEMINI_MAPPING},
