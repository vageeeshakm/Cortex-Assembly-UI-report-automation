...
# # DAG WILL BE SCHEDULED AT 5PM IST From Monday to Friday
dag = DAG('google_analytics_dag',
          default_args=ARGUMENTS.DEFAULT_ARGUMENTS,
          schedule_interval='00 17 * * 1-5', concurrency=2, catchup=False)
...
create_consolidated_report = create_python_operator(
    ...
    op_kwargs={'platform': PLATFORM.GOOGLE_ANALYTICS},
...
try:
    for each in input_config_list:
        ...
        download_google_analytics_report_task = create_python_operator(
            dag,
            f"download_google_analytics_report_task_{each['report_id']}",
            google_analytics_report_object.download_combine_and_upload_reports,
            op_kwargs={'report_config': each, 'platform': PLATFORM.GOOGLE_ANALYTICS},
            provide_context=True
        )
