...
# It should run once in a week
dag = DAG('linkedin_weekly_dag',
          default_args=ARGUMENTS.DEFAULT_ARGUMENTS,
          schedule_interval='00 14 * * 5', concurrency=2, catchup=False)

# start and end dummy operator task to indicate start and end process.
dummy_task_start = create_dummy_operator(dag, 'start_linkedin_weekly_task')
dummy_task_end = create_dummy_operator(dag, 'end_linkedin_weekly_task')
...
email_consolidated_report = create_python_operator(
    dag,
    'email_linkedin_weekly_consolidated_report',
    consolidate_and_validate_object.download_consolidated_report_and_send_email,
    op_kwargs={'platform': PLATFORM.LINKEDINWEEKLY})

# Read the configuration file
input_config_list = read_json_file(file_path=PATH.WEEKLY_REPORT_JSON_FILE_PATH)
...
try:
    for each in input_config_list:
        ...
        download_linkedin_report_task = create_python_operator(
            dag,
            f"download_linkedin_weekly_report_task_{each['account_id']}",
            linkedin_report_object.download_combine_and_upload_reports,
            op_kwargs={'report_config': each, 'platform': PLATFORM.LINKEDINWEEKLY},
